...
optionalExpr: expr | OPTIONAL;

expr:
	exprAtom ('[' (datasetClause | (expr ASSIGN expr)) ']')* (
...
defHierarchical:
	defineHierarchicalRuleset rulesetID '(' hierRuleSignature ')' IS ruleClauseHierarchical
...
valueDomainSignature:
	CONDITION IDENTIFIER (AS IDENTIFIER)? (
		',' IDENTIFIER (AS IDENTIFIER)?
	)*;

codeItemRelation:
	(WHEN expr THEN)? codeItemRef codeItemRelationClause (
...
defDatapoint:
	defineDatapointRuleset rulesetID '(' rulesetSignature ')' IS ruleClauseDatapoint
		endDatapointRuleset;
...
varSignature: varID (AS IDENTIFIER)?;

/* Artefacts Definition */
defExpr: defOperator | defDatapoint | defHierarchical;

defOperator:
	DEFINE OPERATOR operatorID '(' (
		parameterItem (',' parameterItem)*
	)? ')' (RETURNS dataType)? IS expr END OPERATOR;
...
lists: '{' constant (',' constant)* '}';

/* eval */
evalExpr:
	EVAL '(' routineName '(' (componentID | constant)? (
		',' (componentID | constant)
	)* ')' (LANGUAGE STRING_CONSTANT)? (
		RETURNS outputParameterType
	)? ')';

/* cast */
castExpr:
	CAST '(' expr ',' (basicScalarType | valueDomainName) (
		',' STRING_CONSTANT
	)? ')';
...
/* time period agg */
timeAggExpr:
	TIME_AGG '(' STRING_CONSTANT (',' (STRING_CONSTANT | '_'))? (
		',' (expr | '_')
	)? (',' (FIRST | LAST))? ')';

/* check */
validationExpr:
	CHECK '(' expr (erCode)? (erLevel)? (IMBALANCE expr)? (
		INVALID
		| ALL
	)? ')';

validationDatapoint:
	CHECK_DATAPOINT '(' expr ',' IDENTIFIER (
		COMPONENTS componentID (',' componentID)*
	)? (INVALID | ALL_MEASURES | ALL)? ')';

validationHierarchical:
	CHECK_HIERARCHY '(' expr ',' IDENTIFIER (
...
anFunctionClause:
	(aggrFunctionName | anFunction)? '(' expr? (',' expr)* OVER '(' (
		partitionByClause
	)? (orderByClause)? (windowingClause)? ')' ')';

partitionByClause: PARTITION BY IDENTIFIER (',' IDENTIFIER)*;

orderByClause:
	ORDER BY componentID (ASC | DESC)? (
...
joinExpr: joinKeyword '(' joinClause (joinBody)? ')';
...
joinBody:
	joinFilterClause? (
		...
		| joinApplyClause
		| joinAggClause
	)? (joinKeepClause | joinDropClause)? joinRenameClause?;

joinCalcClause:
	componentRole? joinCalcClauseItem (',' joinCalcClauseItem)*;

joinCalcClauseItem:
	CALC (componentRole)? joinCalcExpr (
		',' (componentRole)? joinCalcExpr
	)*;

joinCalcExpr: componentID ':=' expr;

joinAggClause:
	componentRole? AGGREGATE joinAggClauseItem (
		',' joinAggClauseItem
	)* groupingClause? havingClause?;

joinAggClauseItem:
	(componentRole)? joinAggExpr (',' joinAggExpr)*;

joinAggExpr: componentID ':=' aggrFunction;

joinKeepClause: KEEP keepClauseItem (',' keepClauseItem)*;

joinDropClause: DROP dropClauseItem (',' dropClauseItem)*;

joinFilterClause: FILTER expr | rulesetID;

joinRenameClause:
	RENAME (componentID MEMBERSHIP)? varID TO (
		componentID MEMBERSHIP
	)? varID (
		',' (componentID MEMBERSHIP)? varID TO (
			componentID MEMBERSHIP
		)? varID
	)*?;
...
getFiltersClause: getFilterClause (',' getFilterClause)*;

getFilterClause: (FILTER? expr);
...
renameClause: varID TO varID (',' varID TO varID)*;
...
calcClauseItem: (componentRole)? componentID ':=' calcExpr;

calcExpr: aggrFunction '(' expr ')' | expr;

dropClause: DROP dropClauseItem (',' dropClauseItem)*;

dropClauseItem:
	componentID
	| (datasetID MEMBERSHIP componentID);

keepClause: KEEP keepClauseItem (',' keepClauseItem)*;
...
unpivotExpr: UNPIVOT varID ',' varID;

pivotExpr: PIVOT varID ',' varID;
...
subscriptExpr:
	persistentDatasetID '[' componentID '=' constant (
		',' componentID '=' constant
	)? ']';

/*Aggregation operators invocation*/
aggrInvocation:
	aggrFunctionName '(' IDENTIFIER (MEMBERSHIP componentID)? (
		',' IDENTIFIER (MEMBERSHIP componentID)?
	)* (groupingClause)? (havingClause)? ')';
...
havingClause: HAVING '('? aggrFunction? expr ')'?;

/* aggregate sequences */

returnAll: RETURN ALL DATA POINTS;
...
exponent: INTEGER_CONSTANT | FLOAT_CONSTANT;

/* Variable, identifiers, constants */
persistentDatasetID: STRING_CONSTANT;
...
joinKeyword: INNER_JOIN | LEFT_JOIN | FULL_JOIN | CROSS_JOIN;

groupKeyword: (GROUP BY) | (GROUP EXCEPT) | (GROUP ALL);
...
componentType2: STRING | INTEGER | FLOAT | BOOLEAN | DATE;
...
prodValueDomains: IDENTIFIER ('*' IDENTIFIER)*;

prodVariables: varID ('*' varID)*;

operatorType:
	inputParameterType ('*' inputParameterType)* '->' outputParameterType;

inputParameterType: scalarType | datasetType | componentType;
...
retainType: BOOLEAN_CONSTANT | ALL;

defineDatapointRuleset: DEFINE DATAPOINT RULESET;

defineHierarchicalRuleset: DEFINE HIERARCHICAL RULESET;

endDatapointRuleset: END DATAPOINT RULESET;

endHierarchicalRuleset: END HIERARCHICAL RULESET;

defineDataStructure: DEFINE DATA STRUCTURE;
